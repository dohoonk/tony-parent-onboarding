...
Rails.application.config.active_record.encryption.tap do |config|
  ...
  config.primary_key = ENV.fetch("ACTIVE_RECORD_ENCRYPTION_PRIMARY_KEY") {
    # Development and test fallback - NEVER use in production
    (Rails.env.development? || Rails.env.test?) ? "dev_primary_key_for_local_only_32chars!" : raise("ACTIVE_RECORD_ENCRYPTION_PRIMARY_KEY not set")
  }
  ...
  config.deterministic_key = ENV.fetch("ACTIVE_RECORD_ENCRYPTION_DETERMINISTIC_KEY") {
    (Rails.env.development? || Rails.env.test?) ? "dev_deterministic_key_32_chars_min!" : raise("ACTIVE_RECORD_ENCRYPTION_DETERMINISTIC_KEY not set")
  }

  # Key derivation salt
  config.key_derivation_salt = ENV.fetch("ACTIVE_RECORD_ENCRYPTION_KEY_DERIVATION_SALT") {
    (Rails.env.development? || Rails.env.test?) ? "dev_key_derivation_salt_32chars_!" : raise("ACTIVE_RECORD_ENCRYPTION_KEY_DERIVATION_SALT not set")
  }

  # Support unencrypted data during migration period
  config.support_unencrypted_data = Rails.env.development? || Rails.env.test?
